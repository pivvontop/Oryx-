...
buttons.forEach(btn => {
    btn.addEventListener('mouseenter', () => {
        ...
        cursorGlow.style.background = 'radial-gradient(circle, rgba(253,241,117,0.4) 0%, rgba(43,59,34,0) 60%)';
    });
    btn.addEventListener('mouseleave', () => {
        cursorGlow.style.transform = 'translate(-50%, -50%) scale(1)';
        cursorGlow.style.background = 'radial-gradient(circle, rgba(253,241,117,0.5) 0%, rgba(43,59,34,0) 60%)';
    });
});
...
function animateParticles() {
    ...
    grad.addColorStop(0, '#2b3b22');
    grad.addColorStop(1, '#1b2615');
    ctx.fillStyle = grad;
    ...
    for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
            ...
            if (dist < 120) {
                ctx.beginPath();
                ctx.strokeStyle = `rgba(253, 241, 117, ${0.15 - dist/800})`;
                ctx.lineWidth = 0.5;
